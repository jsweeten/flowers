...
const inexpensiveFlowers = () => {
    ...
    // iterate through flowers array and add flowers with price <$2 to a new array
...
const zoneFlowers = () => {
    ...
    // iterate through usdaZones array within flowers object properties
    // and add flowers with usdaZones of 3 and 8 to a new array
...
const orangeFlowers = () => {
    ...
    // iterate through flowers array and add any flower that
    // has "Orange" element within flowers.colors to a new array 
...
const flowersAsHTML = () => {
    ...
    /* Use string interpolation in a loop to write the HTML
    and plug in elements accordingly. Going to need to access
    nested properties using dot notation and join arrays into
    strings. */

    
    flowers.forEach(flower =>   {      
        let availableColors = flower.colors.join("</section></section>")
        let growingRegion = flower.usdaZones.join("</div></div>")
        flowersHTMLString += `<article>
            <h1>${flower.type}</h1>

            <h2>Colors</h2>
            <section>${availableColors}</section>

            <h2>USDA Zones</h2>
            <div>${growingRegion}</div>
        </article>`})
